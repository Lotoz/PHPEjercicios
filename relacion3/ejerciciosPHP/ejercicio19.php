<?php
// ejercicio19.php - versión con probabilidad (tercera opción x2) y mostrar imagen del primer plato
$menu = [
  'entrante' => ['Ensalada César', 'Hummus', 'Boquerones al natural'],
  'primero' => ['Gazpachuelo', 'Salmorejo', 'Ajo Blanco'],
  'segundo' => ['Fritura Malagueña', 'Conejo al ajillo', 'Pisto con huevo'],
  'postre' => ['Helado 3 sabores', 'Flan', 'Tarta de Queso']
];

// images for primeros (corresponding to each option)
$img = [
  'Gazpachuelo' => 'img/gazpachuelo.jpg',
  'Salmorejo' => 'img/salmorejo.jpg',
  'Ajo Blanco' => 'img/ajoblanco.jpg'
];

$n = intval($_GET['n'] ?? 3);
$menus = [];
for ($i = 0; $i < $n; $i++) {
  $m = [];
  foreach ($menu as $tipo => $opciones) {
    if ($tipo === 'primero') {
      // make third option twice as likely
      $choices = [$opciones[0], $opciones[1], $opciones[2], $opciones[2]];
      $sel = $choices[array_rand($choices)];
    } else {
      $sel = $opciones[array_rand($opciones)];
    }
    $m[$tipo] = $sel;
  }
  $menus[] = $m;
}
?>
<!doctype html>
<html>

<head>
  <meta charset="utf-8">
  <title>Ejercicio 19</title>
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
</head>

<body class="p-3">
  <h1>Ejercicio 19 - probabilidad y imagen</h1>
  <div class="row">
    <?php foreach ($menus as $idx => $m): ?>
      <div class="col-md-4">
        <div class="card mb-3">
          <div class="card-body">
            <h5 class="card-title">Menú <?php echo $idx + 1; ?></h5>
            <?php $firstCourse = $m['primero'];
            if (isset($img[$firstCourse])): ?>
              <img src="<?php echo htmlspecialchars($img[$firstCourse]); ?>" alt="" class="img-fluid mb-2">
            <?php endif; ?>
            <ul>
              <?php foreach ($m as $tipo => $plato): ?>
                <li><strong><?php echo ucfirst($tipo); ?>:</strong> <?php echo htmlspecialchars($plato); ?></li>
              <?php endforeach; ?>
            </ul>
          </div>
        </div>
      </div>
    <?php endforeach; ?>
  </div>
  <script src="ejercicio19.js"></script>
</body>

</html>