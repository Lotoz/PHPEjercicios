 ...
 <!--
//Analiza la cadena string, si es invalida devuelve null
        function purifacador($cadena)
        {
            //Limpiamos la entrada
            $san = strip_tags($cadena);
            $san = htmlspecialchars($san, ENT_QUOTES, 'UTF-8');

            // Separar y limpiar cada nombre
            $parts = array_map('trim', explode(',', $san));

            // Validar que cada nombre solo contenga letras, espacios, acentos y guiones
            $valid = true;
            foreach ($parts as $p) {
                if ($p === '') continue;
                if (!preg_match('/^[\p{L}\s\-]+$/u', $p)) {
